...
#   --gdp-gateway-fixture  Enable GDP Browse with recorded fixtures (PageConfig)
...
# If ~/gdp/cacert/cacert.pem exists it is used as --ca-bundle; otherwise certifi alone.
# Pass --ca-bundle / --corp-ca explicitly when needed.
...
NPM_REGISTRY=""
PYTHON_BIN=""
# Optional default: use ~/gdp/cacert/cacert.pem only when present.
# Do not hard-fail when missing — configure_ssl falls back to certifi.
CA_BUNDLE=""
CORP_CA=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --gdp-gateway-fixture) GDP_GATEWAY_FIXTURE=1; shift ;;
...
if [[ "${GDP_GATEWAY_FIXTURE}" -eq 1 ]]; then
  LAB_EXTRA_ARGS+=(--config="${REPO_ROOT}/examples/jupyter_gdp_gateway_pageconfig.py")
fi
...
if [[ "${START_LAB}" -eq 1 ]]; then
  ...
  if [[ "${GDP_GATEWAY_FIXTURE}" -eq 1 ]]; then
    log "  GDP Gateway Browse: fixture mode (examples/jupyter_gdp_gateway_pageconfig.py)"
  fi
  ...
  if [[ "${GDP_GATEWAY_FIXTURE}" -eq 1 ]]; then
    printf ' --config=%s/examples/jupyter_gdp_gateway_pageconfig.py' "${REPO_ROOT}"
  fi
  printf '\n'
  printf '  # GDP Browse fixture (UI only):\n'
  printf '  # jupyter lab --config=%s/examples/jupyter_gdp_gateway_pageconfig.py\n' "${REPO_ROOT}"
